...
export const changeStringToNumberArray = string => string.split('').map(item => Number(item));
...
export const validateNumberInArray = array => {
	...
	return !isNumberArray.includes(true);
};
...
export const createResultHintString = (count, hint) => (count ? `${count}${hint}` : '');
