 ...
 * @description Baseball Game method for game playing
 */
export default function BaseballGame() {
  ...
  this.setAnswer = function() {
    ...
    while (answer.length < 3) {
      const num = String(this.getRandomInt(1, 9));
  ...
  this.checkUserInput = function(userInputNumbers) {
    return (this.isValidNumber(userInputNumbers) &&
              !this.isDuplicated(userInputNumbers));
  };
  ...
  this.play = function(computerInputNumbers, userInputNumbers) {
    const {strike, ball} =
      this.countStrikeAndBall(computerInputNumbers, userInputNumbers);
