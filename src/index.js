export default class BaseballGame {
    ...
    #isInRandomNumbers (randomNumbers,oneNumber) {
        if (randomNumbers.includes(oneNumber)) {
            return true;
        }
    ...
    #isAllDifferent (input) {
        ...
        if (fillterdInput.length !== input.length) {
            return false;
        }
    ...
    #calculateResult (index,computerInputNumbers, userInputNumbers) {
    ...
    // * 정답을 맞췄는지 확인하기
    #isRight (computerInputNumbers, userInputNumbers) {
    ...
    #addNewGameHTML () {
        ...
        const newChild = '<div id = "game-restart-div"></br><span id="restart-span">게임을 새로 시작하시겠습니까?</span>  <button id="game-restart-button">게임 재시작</button></div>';
    ...
    #restart () {
        const parent = document.querySelector("#result");
        const restartDiv = document.querySelector('#game-restart-div');
    ...
    // * 다시하기 버튼에 restart 이벤트 리스너 추가하기
     #addEventToRestartButton () {
         const button = document.querySelector("#game-restart-button");
