...
- [x] 상대방(컴퓨터)은 1부터 9까지 서로 다른 수로 이루어진 3자리의 수를 생성한다.
  - [x] 1부터 9까지 난수 생성 'getRandomIntInclusive'
  - [x] 서로다른 난수로 이루어진 3자리의 수 생성 'getComputerNumbers'
- [x] 사용자(input)로부터 입력값 받기
  - [x] 입력값 받기 'handleSubmit'
  - [x] 입력값 유효성 판단 'isValuedInputNumber'
  - [x] 입력값 중복된 숫자 판단 'isCorrectNumberOfDigits'
- [x] 유저가 입력값을 상대방(컴퓨터)과 비교하여 String값으로 반환한다. 'play'
  - [x] 오답일 경우 같은 수가 같은 자리에 있으면 `스트라이크`, 다른 자리에 있으면 `볼`, 같은 수가 전혀 없으면 `낫싱`이란 힌트를 반환한다. 'incorrectAnswer'
  - [x] 정답일 경우 값을 반환한다 'correctAnswer'
- [x] play함수 결과값으로 화면 그리기 'render'
- [x] 게임종료시 게임 재시작 버튼 설정 'gameRestartButton'
  - [x] 확인버튼 비활성화
- [x] 화면 리셋 'reset'
