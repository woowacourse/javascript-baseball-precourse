...
- ~1 에서 9 까지의 랜덤한 숫자를 생성하는 메소드~
- ~3개의 중복되지 않는 랜덤 숫자 배열을 반환하는 메소드~
- ~사용자 입력을 받아 배열을 반환하는 메소드~
- ~올바른 사용자 입력을 확인하는 메소드~
- ~잘못된 입력시 alert()를 통해 오류 메시지 출력하는 메소드~
- ~사용자의 정답 결과를 확인하는 메소드~
- ~볼 갯수를 반환하는 메소드~
- ~스트라이크 갯수를 반환하는 메소드~
- ~사용자의 정답 결과에 따라 메시지를 반환하는 메소드~
- ~사용자의 정답 결과를 화면에 출력하는 메소드~
- ~게임 재시작 메소드~
- ~사용자가 다음 입력으로 넘어간 경우 이전 입력 칸 disable 시키기~
