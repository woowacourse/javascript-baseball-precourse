...
## 구현할 기능 목록
- 1부터 9까지 중 무작위로 하나의 숫자를 반환하는 메서드 `getRandomNumber`
- 1부터 9까지 서로 다른 수로 이루어진 3자리의 수를 반환하는 메서드 `computingNumber`
- 숫자들을 비교하기 쉽도록 숫자를 받아 각 자리 수의 요소들이 들어가 있는 배열을 반환하는 메서드 `parseNumber`
- 입력이 valid한 값인지 검증하는 메서드 `checkInput`
- 스트라이크, 볼, 정답, 낫싱을 구별하여 String을 반환하는 메서드 `play`
- '확인' 버튼에 이벤트리스너로 추가되어 입력을 검증하고 정답시 재시작 버튼에 이벤트리스너를 추가해주는 메서드 `handleClick`
- '재시작' 버튼에 이벤트리스너로 추가되어 숫자를 다시 뽑고 입력란과 결과란을 비워주는 메서드 `handleRestart`
