...
## 🤔 구현할 기능 목록
- [X] 게임이 시작되면 컴퓨터는 1 ~ 9 사이의 숫자 중 서로 다른 3개의 숫자를 선택하여 3자리 숫자를 정한다.
- [X] 확인 버튼을 누르면 사용자 입력이 유효한지 판단한다.
  - [X] 사용자 입력이 3자리 숫자인지 판단한다.
  - [X] 사용자 입력에 문자가 포함되어 있는지 판단한다.
  - [X] 사용자 입력에 중복되는 숫자가 있는지 판단한다.
  - [X] 유효하지 않다면 경고 메세지를 띄운다.
- [X] 유효한 입력일 경우 `play` 메소드를 통해 게임이 진행된다.
  - [X] ball의 개수를 구한다.
  - [X] strike의 개수를 구한다.
- [] ball과 strike의 개수를 이용해 게임을 진행한다.
  - [X] 먼저 strike의 개수로 정답인지 판단한다.
  - [X] 정답일 경우 정답 메세지와 `게임 재시작` 버튼을 보여준다.
  - [X] ball과 strike가 모두 0이면 `낫띵`을 보여준다.
  - [X] 이 외에는 ball과 strike를 화면에 보여준다.
- [X] 정답일 경우 게임을 재시작 할 수 있다.