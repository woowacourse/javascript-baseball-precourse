...
- 구현할 기능 목록 추가
- 1-9 중에 서로 다른 임의의 수 3개 선택하는 메소드 생성
- 플레이어가 확인버튼 누르면 호출되는 이벤트 리스너 생성
- 플레이어가 입력한 숫자 유효성 검사 후 `alert` 띄워주고 새로 입력할 수 있게 하는 메소드 생성
- 플레이어가 입력한 숫자와 선택된 숫자 비교해서 결과를 나타내는 메소드 생성
- 앱 처음 켰을 때와 재시작할 때 실행할 playNewGame 메소드 생성
- 게임 종료 후 재시작 버튼 보여주는 기능 추가
- `play` 메소드가 number 타입도 받을 수 있도록 처리
