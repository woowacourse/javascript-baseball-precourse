...
**사전 정의**
- [x] 1. button `submit`의 클릭 이벤트에 따른 동작을 설정합니다.
- [x] 2. 옳지 못한 값(3자리 숫자를 제외한 다른 타입과 문자)에 대한 예외를 처리합니다.
- [x] 3. 2번의 확장입니다. 입력 받는 데이터는 text 형태이므로 문자를 숫자로 치환합니다.
- [x] 4. 입력된 3자리 숫자가 중복 되었는지 확인합니다.
- [x] 5. 구현한 기능을 리팩토링합니다.

**추가 사항**
- [x] 버튼 클릭시 기존 이벤트(submit)를 취소합니다.
- [x] 공백이거나 정해진 자리수가 아니라면 예외 처리합니다.
- [x] 예외가 발생하면 input #user-input을 비웁니다.
- [x] 버튼 이벤트가 종료되면 input #user-input에 포커싱합니다.
- [x] 0은 허용되지 않으므로 예외 처리합니다.
...
**사전 정의**
- [x] 1. MissionUtils 라이브러리를 의존하는 컴포넌트를 작성합니다.
- [x] 2. 인자로 자리수를 입력 받아 컴퓨터 랜덤 값을 설정합니다.

**추가 사항**
- [x] 중복되지 않는 랜덤 값을 인자로 받은 자리수까지 이어 타겟 넘버를 생성합니다.
- [x] MissionUtils.Random.pickNumberInRange 함수의 인자를 설정할 수 있게 수정합니다.
- [x] 생성자 함수의 파라미터로 자리수, 시작 숫자, 종료 숫자를 받을 수 있게 작성합니다.
- [x] 입력받은 상태 값을 검증하는 함수를 작성합니다.
- [x] 조건에 맞게 상수 값을 리팩토링 합니다.
...
**사전 정의**
- [x] 1. 입력 값과 비교하여 결과(스트라이크와 볼, 낫싱)를 체크합니다.
- [x] 2. 구현한 기능을 리팩토링합니다.

**추가 사항**
- [x] 볼은 다른 자리에 동일한 값을 체크합니다.
- [x] 스트라이크는 동일한 자리에 동일한 값을 체크합니다.
- [x] 공백 문자열도 상수 처리합니다.
...
**사전 정의**
- [x] 1. 게임 결과에 따라 div #game-restart-button의 텍스트를 갱신시킵니다. innerHTML을 사용해 처리합니다.
- [x] 2. 승리할 경우 재시작 버튼을 클릭하여 게임이 리셋될 수 있어야 하므로 타겟 dom에 대한 정보를 컴포넌트가 인지하고 있어야 합니다. 따라서 컴포넌트의 파라미터를 수정합니다.
- [x] 3. 맞춘 횟수가 야구게임의 숫자 자리수와 같으면 승리입니다.
- [x] 4. 스트라이크와 볼을 더한 수치가 0이라면 아무것도 맞추지 못햇으므로 낫싱입니다.
- [x] 5. 그 외라면 볼과 스트라이크를 차례대로 출력합니다. 단, 1이상인 값만 출력합니다.
