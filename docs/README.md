...
**사전 정의**
- [x] ~~1. 게임 결과에 따라 div #result의 텍스트를 갱신시킵니다. innerHTML을 사용해 처리합니다.~~/ 완료
- [x] ~~2. 승리할 경우 재시작 버튼을 클릭하여 게임이 리셋될 수 있어야 하므로 타겟 dom에 대한 정보를 컴포넌트가 인지하고 있어야 합니다. 따라서 컴포넌트의 파라미터를 수정합니다.~~/ 완료
- [x] ~~3. 맞춘 횟수가 야구게임의 숫자 자리수와 같으면 승리입니다.~~/ 완료
- [x] ~~4. 스트라이크와 볼을 더한 수치가 0이라면 아무것도 맞추지 못햇으므로 낫싱입니다.~~/ 완료
- [x] ~~5. 그 외라면 볼과 스트라이크를 차례대로 출력합니다. 단, 1이상인 값만 출력합니다.~~/ 완료

**추가 사항**
- [x] ~~재시작의 경우 이벤트 위임을 통해 단 1회만 이벤트를 등록하도록 기능을 구현합니다.~~/ 완료
